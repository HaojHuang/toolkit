...
viewpoint_cameras = ["NP1"]	# Camera which the viewpoint will be generated.
viewpoint_angles = [str(i) for i in range(0,3,3)] # Relative angle of the object w.r.t the camera (angle of the turntable).
...
object_list= ['003_cracker_box','006_mustard_bottle','011_banana','021_bleach_cleanser','035_power_drill','048_hammer','072-c_toy_airplane','072-h_toy_airplane']
...
def registerDepthMap(unregisteredDepthMap,
                     ...
                     rgbK,
                     H_RGBFromDepth):
    print(unregisteredDepthMap.shape)
    plt.imshow(unregisteredDepthMap)
    plt.show()
    plt.imshow(rgbImage)
    plt.show()
    ...
    # print(registeredDepthMap.shape)
    plt.imshow(registeredDepthMap)
    plt.show()
    ...
    for target_object in object_list:
        ...
        for viewpoint_camera in viewpoint_cameras:
            ...
            for viewpoint_angle in pbar:
                ...
                #print('calibration',list(calibration.keys()))
                ...
                registeredDepthMap = registerDepthMap(unregisteredDepthMap,
                                                    ...
                                                    rgbK,
                                                    H_RGBFromDepth)
                depth_path = os.path.join(image_path,target_object)
                if not os.path.exists(depth_path):
                    os.makedirs(depth_path)
                #plt.savefig(depth_path+'/'+'_depth')
                #plt.show
                pl.imshow(mask)
                plt.show()
                print(rgbImage.shape)
                #plt.savefig(depth_path+'/'+'_rgb')
                #plt.show()
                continue
